...
def create_app(test_config=None):
  ...
  '''
    ...
    def get_paginated_questions():
        ...
        categories = [
            category.type for category in Category.query.order_by(
                Category.id).all()]
        categories = categories_dict()
  ...
  '''
# Search requests goes to ('questions/') with a search word and handled by
# create_new_question()
    '''
  @TODO:
    ...
    def get_category_questions(category_id):
        try:
            ...
            current_questions = paginate_questions(
                request, current_category_questions)
            return jsonify({
                'success': True,
  ...
  '''
    ...
    def play():
        ...
        if quiz_category['id'] == 0:
            ...
            not_asked_questions = Question.query.filter(
            ...
            current_category = Category.query.filter(
                Category.id == quiz_category['id']).one_or_none()
            current_category_questions = current_category.questions
            not_asked_questions = [
                question for question in current_category_questions
